...
def _all_fields(model: type[models.Model]) -> list[str]:
    all_fields = [
        field.attname for field in _model_fields(model) if not isinstance(field, models.AutoField)
    ]
    return all_fields
...
class _When:
    ...
    def evaluate(self) -> str:
        sql = f"WHEN {self.condition} {self.by} THEN"
...
def _compile_merge(on: _MergeOn[_M], *, returning: list[str] | bool = False) -> _CompiledMerge:
    ...
    sql = "MERGE INTO {table} target USING (VALUES {values_sql}) AS source ({value_fields_sql}) "
    ...
    with connections[using.builder.into.db].cursor() as cursor:
        row_values, sql_args = _get_values_for_rows(
            ...
            _model_fields(using.builder.into.model),
        )
        sql.format(
            table=_quote(using.builder.into.model._meta.db_table, cursor),
            values_sql=row_values,
            value_fields_sql=first_on.fields,
        )
...
class _WhenNotMatched(Generic[_M]):
        ...
        return dataclasses.replace(
            ...
            whens=[
                *self.merge_on.whens_not_matched,
                _When(condition="MATCHED", by=self.by, then=_Insert()),
            ],
        )
    ...
    def do_nothing(self) -> _MergeOn[_M]:
        return dataclasses.replace(
            ...
            whens=[
                *self.merge_on.whens_not_matched,
                _When(condition="MATCHED", by=self.by, then=_DoNothing()),
            ],
        )
